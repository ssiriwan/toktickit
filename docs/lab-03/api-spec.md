...
- Query: `search` (matches `ticketNumber` + `summary`, case-insensitive), `status` (8 values), `categoryId`, `reqPriority`, `itPriority`, `owner` (`me|unassigned|<userId>`), `sort` (`createdDate|updatedAt|reqPriority|itPriority`, default `updatedAt`), `order` (`asc|desc`, default `desc`), `page` (≥1, default 1), `pageSize` (1..50, default 10).
