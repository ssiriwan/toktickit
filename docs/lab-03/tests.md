...
> Written before implementation (Phase 2, Issue #33). Every Acceptance Criterion (AC-01..AC-18 in `specification.md`) maps to ≥1 planned test below. `Final` column is updated to `Pass` only from `lab3-staging` green runs. File paths match labsheet §12 exactly.
...
| UNIT-01 | Unit | AC-01 | bcrypt hashing (cost 12, compare true/false, never plaintext) | hash ≠ password, verify ok | `server/tests/lab-03/auth.api.test.ts` | Planned |
| AUTHZ-01 | API/security | AC-03 | Requester sends forged requesterId / чуж ticket id | session identity applied, 403, no cross data | `server/tests/lab-03/authorization.api.test.ts` | Planned |
...
| DETAIL-06 | API | AC-08 | Requester appears-resolved (own / чуж / idempotent) | 200 flag, status same / 403 / repeat ok | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Planned |
...
| UI-06 | UI/style | AC-18 | Zen Green tokens, badges, readonly vs editable, validation placement, focus | style assertions | `client/tests/lab-03/StaffTicketDetail.test.tsx` | Planned |
...
- [ ] Role nav + direct-API authz + regression header-ignore (AUTHZ-01..06)
...
- [ ] Migration (itPriority copy, ownership, attachments) + Lab 2 regression green
- [ ] Responsive (queue cards, drawer sheet, detail 1-col) + a11y (labels, alert/status roles, focus, keyboard)
