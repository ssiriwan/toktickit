...
- Ticket lifecycle beyond creation: status changes beyond Open, resolution, closing, reopening, cancelling
...
- **BR-02** A new Ticket begins with Current Status set to Open.
...
| currentStatus | TicketStatus | @default(OPEN) |
...
- **AC-01** Given valid ticket data, when the Requester submits the Create Ticket form, then one Ticket is saved with status Open and the official Ticket Number is displayed.
...
- **Status Model**: Lab 2 uses 7 statuses: Open (Sky Blue #E0F2FE), Assigned (Indigo #E0E7FF), In Progress (Purple #F3E8FF), On hold (Amber #FEF3C7), Pending for Approval (Orange #FFEDD5), Monitoring (Teal #CCFBF1), Closed (Slate #F1F5F9). New tickets default to Open. The enum is designed to be extended in Lab 3 if needed.
